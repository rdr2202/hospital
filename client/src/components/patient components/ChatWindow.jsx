const labelStyles = {
  Acute: "bg-[#fcdede] text-[#901c1c]",
  Opinion: "bg-[#e0e5ff] text-[#243189]",
};

const ChatWindow = ({ selectedDoctor }) => {
  if (!selectedDoctor) {
    return (
      <section className="flex flex-col flex-1 bg-white rounded-xl my-4 mr-6 ml-0 justify-center items-center">
        <p className="text-gray-400">Select a patient to start chatting</p>
      </section>
    );
  }

  const messages = selectedDoctor.messages || [];
  ...
  return (
    <section className="flex flex-col flex-1 bg-white rounded-xl my-4 mr-6 ml-0">
      <header className="flex justify-between items-start py-4 px-6 border-b border-gray-200">
        <div>
          <span className="text-lg font-semibold mr-2">{selectedDoctor.name}</span>
      ...
      <div className="flex-1 px-6 py-6 overflow-y-auto">
        {messages.map((msg, idx) => (
          <div
            key={idx}
            className={`mb-5 max-w-[65%] flex flex-col ${
              msg.role === "doctor" ? "self-end" : "self-start"
            }`}
          >
            <div
              className={`p-4 rounded-lg ${
                msg.role === "doctor" ? "bg-[#fbeffa]" : "bg-[#f8eaea]"
              }`}
            >
              ...
              {msg.label && (
                <span
                  className={`ml-2 inline-block px-2 py-0.5 rounded text-xs mt-2 ${
                    labelStyles[msg.label]
                  }`}
                >
                  {msg.label}
                </span>
